...
static void prep_conversion(const Babl* babl)
{
    ...
    matrixf = babl_calloc(sizeof(float), 9 + 256 * 3);
...
#define TRC_IN(rgba_in, rgba_out)                                                                                                       \
    do {                                                                                                                                \
        int32_t i;                                                                                                                      \
        for (i = 0; i < samples; i++) {                                                                                                 \
            rgba_out[i * 4 + 3] = rgba_in[i * 4 + 3];                                                                                   \
        }                                                                                                                               \
        if ((source_space->space.trc[0] == source_space->space.trc[1]) && (source_space->space.trc[1] == source_space->space.trc[2])) { \
...
#define TRC_OUT(rgba_in, rgba_out)                                                                                                                              \
    do {                                                                                                                                                        \
        {                                                                                                                                                       \
            if ((destination_space->space.trc[0] == destination_space->space.trc[1]) && (destination_space->space.trc[1] == destination_space->space.trc[2])) { \
...
static inline void universal_nonlinear_rgba_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    const Babl* source_space = babl_conversion_get_source_space(conversion);
...
static inline void universal_nonlinear_rgb_linear_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    const Babl* source_space = babl_conversion_get_source_space(conversion);
    float* matrixf = data;
    float* rgba_in = (void*)src_char;
...
static inline void universal_linear_rgb_nonlinear_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
...
static inline void universal_rgba_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    float* matrixf = data;
    float* rgba_in = (void*)src_char;
    float* rgba_out = (void*)dst_char;
...
static inline void universal_rgb_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    float* matrixf = data;
...
static inline void universal_ya_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    memcpy(dst_char, src_char, samples * 4 * 2);
}

static inline void universal_y_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    memcpy(dst_char, src_char, samples * 4);
}

static inline void universal_nonlinear_rgb_u8_converter(const Babl* conversion, uint8_t* __restrict__ src_char, uint8_t* __restrict__ dst_char, long samples, void* data)
{
    const Babl* destination_space = conversion->conversion.destination->format.space;
...
static int add_rgb_adapter(Babl* babl, void* space)
{
    if (babl != space) {

#if defined(USE_SSE2)
        if ((babl_cpu_accel_get_support() & BABL_CPU_ACCEL_X86_SSE) && (babl_cpu_accel_get_support() & BABL_CPU_ACCEL_X86_SSE2)) {
...
void BABL_SIMD_SUFFIX(_babl_space_add_universal_rgb)(const Babl* space);
void BABL_SIMD_SUFFIX(_babl_space_add_universal_rgb)(const Babl* space)
{
