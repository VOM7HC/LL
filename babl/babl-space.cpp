    ...
    double* to_b)
{
    ...
    if (x_r > LAB_EPSILON)
        ...
        (f_x = ((LAB_KAPPA * x_r) + 16) / 116.0);

    if (y_r > LAB_EPSILON)
        f_y = pow(y_r, 1.0 / 3.0);
    else
        (f_y = ((LAB_KAPPA * y_r) + 16) / 116.0);

    if (z_r > LAB_EPSILON)
        f_z = pow(z_r, 1.0 / 3.0);
    else
        (f_z = ((LAB_KAPPA * z_r) + 16) / 116.0);
...
babl_matrix_equalize(double* in_mat)
{
    ...
    for (int i = 0; i < sizeof(equalized_matrices) / sizeof(equalized_matrices[0]); i++) {
...
Babl* _babl_space_for_lcms(const char* icc_data,
    int icc_length)
{
    int i = 0;
    BablSpace space = {
    ...
    for (i = 0; space_db[i].instance.class_type; i++) {
        if (space_db[i].icc_length == icc_length && (memcmp(space_db[i].icc_profile, icc_data, icc_length) == 0)) {
    ...
    /* initialize it with copy of srgb content */
    {
        ...
        memcpy(&space.xw,
            &srgb->xw,
            ((char*)&srgb->icc_profile - (char*)&srgb->xw));
    }
...
void babl_space_class_for_each(BablEachFunction each_fun,
    void* user_data)
{
    int i = 0;
    for (i = 0; space_db[i].instance.class_type; i++)
...
void babl_space_to_xyz(const Babl* space,
    const double* rgb,
    double* xyz)
{
...
void babl_space_from_xyz(const Babl* space,
    const double* xyz,
    double* rgb)
{
    ...
    float bx, float by, float bz)
{
    ...
    for (i = 0; space_db[i].instance.class_type; i++) {
        ...
        if (space->icc_type == BablICCTypeRGB && trc_red == space->trc[0] && trc_green == space->trc[1] && trc_blue == space->trc[2] && fabs(rx - space->RGBtoXYZ[0]) < delta && fabs(ry - space->RGBtoXYZ[3]) < delta && fabs(rz - space->RGBtoXYZ[6]) < delta && fabs(gx - space->RGBtoXYZ[1]) < delta && fabs(gy - space->RGBtoXYZ[4]) < delta && fabs(gz - space->RGBtoXYZ[7]) < delta && fabs(bx - space->RGBtoXYZ[2]) < delta && fabs(by - space->RGBtoXYZ[5]) < delta && fabs(bz - space->RGBtoXYZ[8]) < delta) {
            return (void*)&space_db[i];
        }
    }
    return NULL;
    ...
    const Babl** blue_trc)
{
    ...
    if (red_trc)
        *red_trc = space->trc[0];
    if (green_trc)
        *green_trc = space->trc[1];
    if (blue_trc)
        *blue_trc = space->trc[2];
}
...
{
    if (space->space.trc[0] != space->space.trc[1] || space->space.trc[1] != space->space.trc[2] || space->space.trc[0]->trc.type != BABL_TRC_FORMULA_GAMMA)
        return 0.0;
