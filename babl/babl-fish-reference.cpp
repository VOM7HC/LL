    ...
    const Babl* destination)
{
    ...
    babl = babl_calloc(1, sizeof(BablFishReference) + strlen(name) + 1);
    ...
    int n)
{
    ...
    dst_img->pitch[0] = (dst_img->type[0]->bits / 8) * source_fmt->model->components;
    dst_img->stride[0] = 0;
...

    {
        ...
        for (i = 0; i < source_fmt->model->components; i++) {
            ...
            dst_img->data[0] = double_buf + (dst_img->type[0]->bits / 8) * i;
            ...
            for (j = 0; j < source_fmt->components; j++) {
                ...
                if (source_fmt->component[j] == source_fmt->model->component[i]) {
    ...
    for (i = 0; i < destination_fmt->components; i++) {
        ...
        if (can_be_used)
            for (j = 0; j < destination_fmt->model->components; j++) {
                if (destination_fmt->component[i] == destination_fmt->model->component[j]) {
                    src_img->data[0] = destination_double_buf + (src_img->type[0]->bits / 8) * j;
    ...
    int n)
{
...

    {
        ...
        for (i = 0; i < source_fmt->model->components; i++) {
            ...
            dst_img->data[0] = float_buf + (dst_img->type[0]->bits / 8) * i;
    ...
    int n)
{
    ...
    for (i = 0; i < destination_fmt->components; i++) {
        ...
        if (can_be_used)
            for (j = 0; j < destination_fmt->model->components; j++) {
                if (destination_fmt->component[i] == destination_fmt->model->component[j]) {
                    src_img->data[0] = destination_float_buf + (src_img->type[0]->bits / 8) * j;
...
#define MAX(a, b) ((a) > (b) ? (a) : (b))
    ...
    if ((babl->fish.source->format.type[0]->bits < 32 || babl->fish.source->format.type[0] == type_float) && (babl->fish.destination->format.type[0]->bits < 32 || babl->fish.destination->format.type[0] == type_float)) {
        void* float_buf = babl_malloc(sizeof(float) * n * MAX(BABL(babl->fish.source)->format.model->components, BABL(babl->fish.source)->format.components));
        ...
        void* double_buf = babl_malloc(sizeof(double) * n * MAX(BABL(babl->fish.source)->format.model->components, BABL(babl->fish.source)->format.components));
#undef MAX
        if (compatible_components((void*)babl->fish.source,
                (void*)babl->fish.destination)) {
            ncomponent_convert_to_double(
    ...
    void* data)
{
    ...
    if (babl->fish.source->format.type[0] == type_double && BABL(babl->fish.source)->format.components == BABL(babl->fish.source)->format.model->components) {
        ...
        source_double_buf = source_double_buf_alloc = babl_malloc(sizeof(double) * n * BABL(babl->fish.source)->format.model->components);
    ...
    case KIND_RGB: {
        ...
        rgba_double_buf = rgba_double_buf_alloc = babl_malloc(sizeof(double) * n * 4);
    ...
    case KIND_CMYK:
        if (babl_model_is((void*)babl->fish.source->format.model, "cmykA")) {
            ...
            cmyka_double_buf = cmyka_double_buf_alloc = babl_malloc(sizeof(double) * n * 5);

            cmyka_image = babl_image_from_linear(
                cmyka_double_buf, babl_remodel_with_space(babl_model("cmykA"), source_space));
    ...
    if (source_kind == KIND_RGB && destination_kind == KIND_RGB) {
    ...
    else if (source_kind == KIND_RGB && destination_kind == KIND_CMYK) {
        cmyka_double_buf = cmyka_double_buf_alloc = babl_malloc(sizeof(double) * n * 5);
        cmyka_image = babl_image_from_linear(
            cmyka_double_buf, babl_remodel_with_space(babl_model("cmykA"), destination_space));
    ...
    else if (source_kind == KIND_CMYK && destination_kind == KIND_RGB) {
        /* */
        rgba_double_buf_alloc = babl_malloc(sizeof(double) * n * 4);
        rgba_double_buf = rgba_double_buf_alloc;
        rgba_image = babl_image_from_linear(
            rgba_double_buf, babl_remodel_with_space(babl_model_from_id(BABL_RGBA), destination_space));
        ...
        if ((babl_space("scRGB") != ((babl->fish.destination)->format.space))) {
            double matrix[9];
            double* rgba = rgba_double_buf;
            babl_matrix_mul_matrix(
    ...
    else if (source_kind == KIND_CMYK && destination_kind == KIND_CMYK) {
        if (source_space != destination_space
#if HAVE_LCMS
...
#define MAX_CMYK_CMYK 64
            ...
            for (cmyk_cmyk_no = 0; cmyk_cmyk_no < cmyk_cmyk_count; cmyk_cmyk_no++) {
                if (cmyk_cmyk_source[cmyk_cmyk_no] == source_space && cmyk_cmyk_destination[cmyk_cmyk_no] == destination_space)
                    break;
            }
...
#define TYPE_CMYKA_DBL (FLOAT_SH(1) | COLORSPACE_SH(PT_CMYK) | EXTRA_SH(1) | CHANNELS_SH(4) | BYTES_SH(0))
    ...
    {
    case KIND_CMYK: {
        const Babl* destination_cmyka_format = babl_remodel_with_space(babl_model("cmykA"), destination_space);
        if (BABL(babl->fish.destination)->format.model == (void*)destination_cmyka_format) {
            destination_double_buf = cmyka_double_buf;
        }
        else {
            Babl* conv = assert_conversion_find(destination_cmyka_format,
                BABL(babl->fish.destination)->format.model);
            destination_double_buf = destination_double_buf_alloc = babl_malloc(sizeof(double) * n * BABL(babl->fish.destination)->format.model->components);
            if (conv->class_type == BABL_CONVERSION_PLANAR) {
    ...
    case KIND_RGB: {
        const Babl* destination_rgba_format = babl_remodel_with_space(babl_model_from_id(BABL_RGBA),
            destination_space);
        ...
        if (BABL(babl->fish.destination)->format.model == (void*)destination_rgba_format) {
            ...
            Babl* conv = assert_conversion_find(destination_rgba_format,
                BABL(babl->fish.destination)->format.model);
            destination_double_buf_alloc = babl_malloc(sizeof(double) * n * BABL(babl->fish.destination)->format.model->components);
    ...
    void* data)
{
...

    {
        ...
        conv_to_rgba = babl_conversion_find(
    ...
    }
    {
        ...
        destination_float_format = babl_format_with_space(dst_name,
            BABL(BABL((babl->fish.destination))->format.space));
        conv_from_rgba = babl_conversion_find(
            babl_format_with_space("RGBA float",
                BABL(BABL((babl->fish.destination))->format.space)),
    ...
    babl_mutex_lock(babl_reference_mutex);
    if (babl->fish.source->format.type[0] == type_float && BABL(babl->fish.source)->format.components == BABL(babl->fish.source)->format.model->components && 0) {
        ...
        source_float_buf_alloc = babl_malloc(sizeof(float) * (n + 1) * (BABL(babl->fish.source)->format.model->components));
...

    {
        if (babl_format_with_space("RGBA float",
            ...
            destination_float_buf_alloc = babl_malloc(sizeof(float) * n * BABL(babl->fish.destination)->format.model->components);
    ...
    void* data)
{
    ...
    if ((BABL(babl->fish.source)->format.model == BABL(babl->fish.destination)->format.model) && (BABL(babl->fish.source)->format.space == BABL(babl->fish.destination)->format.space)) {
    ...
    if (format_has_cmyk_model(babl->fish.source) || format_has_cmyk_model(babl->fish.destination)) {
        babl_fish_reference_process_double(babl, source, destination, n, data);
        return;
    }
    ...
    if (allow_float_reference && (babl->fish.source->format.type[0]->bits < 32 || babl->fish.source->format.type[0] == type_float) && (babl->fish.destination->format.type[0]->bits < 32 || babl->fish.destination->format.type[0] == type_float) && !babl_format_is_palette(babl->fish.source) && !babl_format_is_palette(babl->fish.destination)) {
