...
#define BABL_POLYNOMIAL_DEGREE(i, i_1)                                                                \
    ...
    {                                                                                                 \
        ...
        (void)x2;                                                                                     \
                                                                                                      \
        return BABL_POLYNOMIAL_EVAL_##i(big_poly, x2) + BABL_POLYNOMIAL_EVAL_##i_1(big_poly, x2) * x; \
    }
#include "babl-polynomial.cpp"
...
#define BABL_POLYNOMIAL_DEGREE(i, i_1)                                                                    \
    ...
    {                                                                                                     \
        ...
        const BablBigPolynomial* const big_poly = (const BablBigPolynomial*)poly;                         \
                                                                                                          \
        return BABL_POLYNOMIAL_EVAL_##i(big_poly, x) + BABL_POLYNOMIAL_EVAL_##i_1(big_poly, x) * sqrt(x); \
    }
#include "babl-polynomial.cpp"
...
babl_polynomial_set_degree(BablPolynomial* poly,
    ...
    int scale)
{
    babl_assert(degree >= BABL_POLYNOMIAL_MIN_DEGREE && degree <= BABL_BIG_POLYNOMIAL_MAX_DEGREE);
    babl_assert(scale >= BABL_POLYNOMIAL_MIN_SCALE && scale <= BABL_POLYNOMIAL_MAX_SCALE);
    ...
    const BablPolynomial* rpoly)
{
    ...
    if (poly->degree >= rpoly->degree) {
        for (i = 0; i <= rpoly->degree; i++) {
            babl_polynomial_set(poly, i, babl_polynomial_get(poly, i) + babl_polynomial_get(rpoly, i));
        }
    }
    else {
        ...
        for (i = 0; i <= orig_degree; i++) {
            babl_polynomial_set(poly, i, poly->coeff[orig_degree - i] + babl_polynomial_get(rpoly, i));
        }
    ...
    const BablPolynomial* poly2)
{
    ...
    for (i = poly1->degree; i >= 0; i--) {
        for (j = poly2->degree; j >= 0; j--) {
            babl_polynomial_set(poly, i + j, babl_polynomial_get(poly, i + j) + babl_polynomial_get(poly1, i) * babl_polynomial_get(poly2, j));
        }
    }
}
    ...
    double x1)
{
    ...
    return babl_polynomial_eval((BablPolynomial*)&temp, x1) - babl_polynomial_eval((BablPolynomial*)&temp, x0);
}
    ...
    double x1)
{
    ...
    return babl_polynomial_eval((BablPolynomial*)&temp, x1) * pow(x1, gamma) - babl_polynomial_eval((BablPolynomial*)&temp, x0) * pow(x0, gamma);
}
...
void babl_polynomial_approximate_gamma(BablPolynomial* poly,
    double gamma,
    double x0,
    double x1,
    int degree,
    int scale)
{
    ...
    babl_assert(x0 >= 0.0);
    babl_assert(x0 < x1);
    babl_assert(degree >= BABL_POLYNOMIAL_MIN_DEGREE && degree <= BABL_POLYNOMIAL_MAX_DEGREE);
    babl_assert(scale >= BABL_POLYNOMIAL_MIN_SCALE && scale <= BABL_POLYNOMIAL_MAX_SCALE);
