...
{
    BablTRC* trc = (void*)trc_;
    if (value >= trc->poly_gamma_to_linear_x0 && value <= trc->poly_gamma_to_linear_x1) {
...
{
    BablTRC* trc = (void*)trc_;
    if (value >= trc->poly_gamma_from_linear_x0 && value <= trc->poly_gamma_from_linear_x1) {
...
static inline void _babl_trc_gamma_to_linear_buf(const Babl* trc_, const float* __restrict__ in, float* __restrict__ out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
...
static inline void _babl_trc_gamma_from_linear_buf(const Babl* trc_, const float* __restrict__ in, float* __restrict__ out, int in_gap, int out_gap, int components, int count)
{
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
            for (int32_t c = 0; c < 3; c++)
...
static inline float _babl_trc_formula_cie_from_linear(const Babl* trc_, float value)
{
    BablTRC* trc = (void*)trc_;
    float x = value;
    float a = trc->lut[1];
...
static inline float _babl_trc_formula_cie_to_linear(const Babl* trc_, float value)
{
    BablTRC* trc = (void*)trc_;
    float x = value;
    float a = trc->lut[1];
...
static inline void _babl_trc_srgb_to_linear_buf(const Babl* trc_, const float* in, float* out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
            for (int32_t c = 0; c < 3; c++)
...
static inline void _babl_trc_srgb_from_linear_buf(const Babl* trc_, const float* in, float* out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
            for (int32_t c = 0; c < 3; c++)
...
static inline void _babl_trc_to_linear_buf_generic(const Babl* trc_, const float* __restrict__ in, float* __restrict__ out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
    BablTRC* trc = (void*)trc_;
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
...
static inline void _babl_trc_from_linear_buf_generic(const Babl* trc_, const float* __restrict__ in, float* __restrict__ out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
    BablTRC* trc = (void*)trc_;
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
...
static inline void _babl_trc_linear_buf(const Babl* trc_, const float* __restrict__ in, float* __restrict__ out, int32_t in_gap, int32_t out_gap, int32_t components, int32_t count)
{
    if (in_gap == out_gap && in_gap == 4 && components == 3) {
        for (int32_t i = 0; i < count; i++)
            for (int32_t c = 0; c < 3; c++)
...
const Babl* BABL_SIMD_SUFFIX(babl_trc_lookup_by_name)(const int8_t* name);
...
const Babl* BABL_SIMD_SUFFIX(babl_trc_new)(const int8_t* name, BablTRCType type, double gamma, int32_t n_lut, float* lut);
...
const Babl* BABL_SIMD_SUFFIX(babl_trc_new)(const int8_t* name, BablTRCType type, double gamma, int32_t n_lut, float* lut)
{
    ...
    if (n_lut) {
        for (i = 0; trc_db[i].instance.class_type; i++) {
            if (trc_db[i].lut_size == n_lut && (memcmp(trc_db[i].lut, lut, sizeof(float) * n_lut) == 0)) {
        ...
        for (i = 0; trc_db[i].instance.class_type; i++) {
            int32_t offset = ((char*)&trc_db[i].type) - (char*)(&trc_db[i]);
            int32_t size = ((char*)&trc_db[i].gamma + sizeof(double)) - ((char*)&trc_db[i].type);
...
void BABL_SIMD_SUFFIX(babl_trc_class_for_each)(BablEachFunction each_fun, void* user_data);
