...
static inline void babl_matrix_mul_matrix(const double* matA_, const double* matB_, double* out)
{
    uint32_t i, j;
    double matA[9];
    double matB[9];
...
static inline void babl_matrix_mul_matrixf(const float* matA_, const float* matB_, float* out)
{
    uint32_t i, j;
    float matA[9];
    float matB[9];
...
static inline void babl_matrix_invert(const double* in, double* out)
{
    ...
    det = m(mat, 0, 0) * (m(mat, 1, 1) * m(mat, 2, 2) - m(mat, 2, 1) * m(mat, 1, 2)) - m(mat, 0, 1) * (m(mat, 1, 0) * m(mat, 2, 2) - m(mat, 1, 2) * m(mat, 2, 0)) + m(mat, 0, 2) * (m(mat, 1, 0) * m(mat, 2, 1) - m(mat, 1, 1) * m(mat, 2, 0));
    invdet = 1.0 / det;
...
static inline void babl_matrix_mul_vectorff_buf3(const float* mat, const float* v_in, float* v_out, uint32_t samples)
{
    uint32_t i;
    const float m_0_0 = m(mat, 0, 0);
    const float m_0_1 = m(mat, 0, 1);
...
static inline void babl_matrix_mul_vectorff_buf4(const float* mat, const float* v_in, float* v_out, uint32_t samples)
{
    const float m_0_0 = m(mat, 0, 0);
    const float m_0_1 = m(mat, 0, 1);
    const float m_0_2 = m(mat, 0, 2);
...
static inline void babl_matrix_mul_vector_buf4(const double* mat, const double* v_in, double* v_out, uint32_t samples)
{
    uint32_t i;
