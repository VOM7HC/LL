...
static int gc_fishes(Babl* babl, void* userdata)
{
    ...
    if (babl->class_type == BABL_FISH_PATH) {
        if (babl->fish_path.u8_lut) {
            if (context->time - babl->fish_path.last_lut_use > 1000 * 1000 * 60 * lut_unused_minutes_limit) {
    ...
    void* data)
{
    ...
    if (lut) {
        if (source_bpp == 4 && ((babl->conversion.source->format.model->flags & BABL_MODEL_FLAG_ASSOCIATED) != 0))
...
char* _babl_fish_create_name(char* buf,
    const Babl* source,
...
_babl_legal_error(void)
{
...

    {
        ...
        if (*((char*)&u32) == 0) { /* disable use of LUTs if we are running on big endian */
            enable_lut = 0;
        }
    }
...
{
    if (babl_format_has_alpha(from) && babl_format_has_alpha(to) && !babl_format_has_alpha(format)) {
        return 1;
    }
    if (from->format.components > format->format.components && to->format.components > format->format.components) {
        return 1;
    }
    if (from->format.type[0]->bits > format->format.type[0]->bits && to->format.type[0]->bits > format->format.type[0]->bits) {
...
int _babl_fish_path_destroy(void* data);
    ...
    void* user_data)
{
    ...
    if ((conv->source->class_type == BABL_FORMAT) && (conv->destination->class_type == BABL_FORMAT) && (!babl_format_is_palette(conv->source)) && (!babl_format_is_palette(conv->destination))) {
        if ((conv->source->format.space == sRGB) && (conv->destination->format.space == sRGB)) {
    ...
    else if ((conv->source->class_type == BABL_MODEL) && (conv->destination->class_type == BABL_MODEL)) {
        if ((conv->source->model.space == sRGB) && (conv->destination->model.space == sRGB)) {
            switch (conv->instance.class_type) {
            case BABL_CONVERSION_LINEAR:
                babl_conversion_new(
    ...
    else if ((conv->source->class_type == BABL_TYPE) && (conv->destination->class_type == BABL_TYPE)) {
    }
    return 0;
}
...
void _babl_fish_prepare_bpp(Babl* babl)
{
    ...
    if (enable_lut) {
        ...
        int src_not_associated = ((babl->conversion.source->format.model->flags & BABL_MODEL_FLAG_ASSOCIATED) == 0);
        int dest_not_associated = ((babl->conversion.destination->format.model->flags & BABL_MODEL_FLAG_ASSOCIATED) == 0);
        if (
            ...
            // currently have built-in expectation for what type alpha is filled in
            {
                ...
                LUT_LOG("%sLUT for %s to %s   %.2f%s%.2f\n",
                    ...
                    ((lut_timing_for(source_bpp, dest_bpp) * scaling) < babl->fish_path.cost) ? "possible " : "no ",

                    babl_get_name(babl->conversion.source),
                    babl_get_name(babl->conversion.destination),
                    (lut_timing_for(source_bpp, dest_bpp) * scaling),
                    ((lut_timing_for(source_bpp, dest_bpp) * scaling) < babl->fish_path.cost) ? " < " : " > ",
                    babl->fish_path.cost);
                if ((lut_timing_for(source_bpp, dest_bpp) * scaling) < babl->fish_path.cost) {
    ...
    double tolerance)
{
    ...
    if ((source->format.space != sRGB) || (destination->format.space != sRGB)) {
    ...
    babl = babl_calloc(1, sizeof(BablFishPath) + strlen(name) + 1);
...
long babl_process(const Babl* babl,
    const void* source,
    void* destination,
    long n)
{
...
long babl_process_rows(const Babl* fish,
    const void* source,
    int source_stride,
    void* dest,
    int dest_stride,
