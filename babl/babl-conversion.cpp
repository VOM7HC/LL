...
Babl* _conversion_new(const char* name,
    int id,
    const Babl* source,
    ...
    int allow_collision)
{
    ...
    babl_assert(source->class_type == destination->class_type);
    ...
    if (babl->class_type == BABL_CONVERSION_LINEAR && BABL(babl->conversion.source)->class_type == BABL_MODEL) {
    ...
    int type)
{
    if (babl_extender()) {
        snprintf(buf, sizeof(buf), "%s %i: %s%s to %s",
            ...
            collisions,
            type == BABL_CONVERSION_LINEAR ? "" : type == BABL_CONVERSION_PLANE ? "plane "
                : type == BABL_CONVERSION_PLANAR                                ? "planar "
                                                                                : "Eeeek! ",
...
babl_conversion_error(BablConversion* conversion)
{
    ...
    if (!(fmt_source->instance.id != BABL_RGBA && fmt_destination->instance.id != BABL_RGBA && fmt_source->instance.id != BABL_DOUBLE && fmt_destination->instance.id != BABL_DOUBLE && fmt_source->class_type == BABL_FORMAT && fmt_destination->class_type == BABL_FORMAT)) {
