    ...
    void* user_data)
{
    ...
    if (list) {
        for (i = 0; i < babl_list_size(list); i++) {
            if (babl_conversion_destination((Babl*)list->items[i]) == babl_model_from_id(BABL_RGBA) || babl_conversion_destination((Babl*)list->items[i]) == babl_model("cmykA")) {
                ok = 1;
                break;
            }
        }
...
id_sanity(Babl* babl,
    void* user_data)
{
    if (0 && 0 == babl->instance.id && babl->instance.creator && !strcmp(BABL(babl->instance.creator)->instance.name, "BablBase")) {
        OK = 0;
