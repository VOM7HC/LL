    ...
    int* stride)
{
    ...
    babl = babl_malloc(sizeof(BablImage) + sizeof(BablComponent*) * (components) + sizeof(BablSampling*) * (components) + sizeof(BablType*) * (components) + sizeof(void*) * (components) + sizeof(int) * (components) + sizeof(int) * (components));
    ...
    const Babl* cformat)
{
    ...
    babl_assert(format);
    babl_assert(format->class_type == BABL_FORMAT || format->class_type == BABL_MODEL);
...
#else
        babl = __atomic_exchange_n(&format->format.image_template, NULL, __ATOMIC_ACQ_REL);
#endif
#else
