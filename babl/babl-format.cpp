    ...
    const char* doc)
{
    ...
    babl = babl_malloc(sizeof(BablFormat) + strlen(name) + 1 + sizeof(BablComponent*) * (components) + sizeof(BablSampling*) * (components) + sizeof(BablType*) * (components) + sizeof(int) * (components) + sizeof(int) * (components));
...
Babl* format_new_from_format_with_space(const Babl* format,
    const Babl* space)
{
    Babl* ret;
    char new_name[256];
    ...
    const BablType** type)
{
    ...
    if (same_types && components != model->components)
        same_types = 0;
    ...
    while (i--) {
        snprintf(p, left, "(%s as %s) ",
            ...
            (*type)->instance.name);
        p += strlen((*component)->instance.name) + strlen((*type)->instance.name) + strlen("( as ) ");
        left -= strlen((*component)->instance.name) + strlen((*type)->instance.name) + strlen("( as ) ");
        babl_assert(left >= 0);
        component++;
        type++;
    }
    ...
    int components)
{
    ...
    BablComponent** component = (BablComponent**)malloc(components * sizeof(BablComponent*));
    BablSampling** sampling = (BablSampling**)malloc(components * sizeof(BablSampling*));
    const BablType** type = (const BablType**)malloc(components * sizeof(const BablType*));
    ...
    const BablType** type)
{
    ...
    if (babl->format.planar != planar || babl->format.components != components || babl->format.model != model || babl->format.space != (void*)space)
        return 0;

    for (i = 0; i < components; i++) {
        if (babl->format.component[i] != component[i] || babl->format.sampling[i] != sampling[i] || babl->format.type[i] != type[i])
            return 0;
    }
    return 1;
}
    ...
    ...)
{
    ...
    if (!name) {
        ...
        if (space != babl_space("sRGB")) {
            char* new_name = babl_malloc(strlen(name) + strlen(babl_get_name((Babl*)space)) + 1);
    ...
    if (babl) {
         ...
         */
        if (0)
            if (!is_format_duplicate(babl, planar, components, model, space,
...
babl_format_get_type(const Babl* format,
    int component_index)
{
    if (format->class_type == BABL_FORMAT && component_index >= 0 && component_index < format->format.components) {
...
void babl_set_user_data(const Babl* cbabl,
    void* data)
{
...
babl_format_get_encoding(const Babl* babl)
{
    if (!babl)
        return NULL;
    if (babl->format.encoding)
        return babl->format.encoding;
