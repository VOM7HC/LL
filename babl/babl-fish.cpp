    ...
    void* data)
{
    ...
    if ((item->fish.source == ffish->source) && (item->fish.destination == ffish->destination)) {
    ...
    void* data)
{
    BablFindFish* ffish = (BablFindFish*)data;
    if ((item->fish.source == ffish->source) && (item->fish.destination == ffish->destination)) {
        if ((item->fish.source == item->fish.destination) && (item->instance.class_type == BABL_FISH_REFERENCE)) {
            ffish->fish_ref = item;
            return 1;
        }
    }
...

    {
        ...
        if (source_format == destination_format) {
            ...
            if (!ffish.fish_fish) {
                ...
                if (!babl_space_is_cmyk(src_space) && !babl_space_is_cmyk(dst_space)) {
