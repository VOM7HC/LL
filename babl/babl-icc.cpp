...
read_s16(ICC* state,
    int offset)
{
    return icc_read(u8, offset + 1) + (read_s8(state, offset + 0) << 8); // XXX: transform to icc_read macro
}

static uint16_t read_u16(ICC* state,
    int offset)
{
    return icc_read(u8, offset + 1) + (icc_read(u8, offset + 0) << 8);
}
...
read_u32(ICC* state,
    int offset)
{
    return icc_read(u8, offset + 3) + (icc_read(u8, offset + 2) << 8) + (icc_read(u8, offset + 1) << 16) + (icc_read(u8, offset + 0) << 24);
}
    ...
    int* ret_length)
{
...

    {
        ...
        if (space->trc[0] == space->trc[1] && space->trc[0] == space->trc[2]) {
    ...
    const char* country)
{
    ...
    for (i = 0; i < n_records; i++) {
        ...
        if (((!lang || !strcmp(lang, ilang)) && (!country || !strcmp(country, icountry))) || (i == n_records - 1)) {
...
#define TYPE_RGBA_DBL (FLOAT_SH(1) | COLORSPACE_SH(PT_RGB) | EXTRA_SH(1) | CHANNELS_SH(3) | BYTES_SH(0))
        ...
        if (!(!strcmp(color_space.str, "RGB ") || !strcmp(color_space.str, "GRAY"))) {
            *error = "not defining RGB, CMYK or GRAY space..";
        }
        else {
            if (!(!strcmp(profile_class.str, "mntr") || !strcmp(profile_class.str, "scnr")))
    ...
    if (!*error)
        ...
        case BABL_ICC_INTENT_RELATIVE_COLORIMETRIC:
            ...
            if (!speed_over_accuracy) {
                if (icc_tag(state, "A2B0", NULL, NULL) && icc_tag(state, "B2A0", NULL, NULL)) {
    ...
    if (is_gray) {
        ...
        *error = "gray parsing NYI";
        ...
        if (icc_tag(state, "rXYZ", NULL, NULL) && icc_tag(state, "gXYZ", NULL, NULL) && icc_tag(state, "bXYZ", NULL, NULL) && icc_tag(state, "wtpt", NULL, NULL)) {
            int offset, element_size;
            double rx, gx, bx;
            double ry, gy, by;
            double rz, gz, bz;
        ...
        else if (icc_tag(state, "chrm", NULL, NULL) && icc_tag(state, "wtpt", NULL, NULL)) {
            int offset, element_size;
    ...
    const char* country)
{
    ...
    if (!strcmp(key, "copyright") || !strcmp(key, "cprt")) {
        ret = decode_string(state, "cprt", language, country);
    }
    else if (!strcmp(key, "description") || !strcmp(key, "profileDescriptionTag") || !strcmp(key, "desc")) {
        ret = decode_string(state, "desc", language, country);
    }
    else if (!strcmp(key, "manufacturer") || !strcmp(key, "deviceMfgDescTag") || !strcmp(key, "dmnd")) {
        ret = decode_string(state, "dmnd", language, country);
    }
    else if (!strcmp(key, "device") || !strcmp(key, "deviceModelDescTag") || !strcmp(key, "dmdd")) {
        ret = decode_string(state, "dmdd", language, country);
    }
    else if (!strcmp(key, "class") || !strcmp(key, "profile-class")) {
